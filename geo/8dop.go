...
func DOP8FromPoints(d *DOP8, points []glm.Vec3) {
	...
	d.Min = [4]float32{math.MaxFloat32, math.MaxFloat32, math.MaxFloat32, math.MaxFloat32}
	d.Max = [4]float32{-math.MaxFloat32, -math.MaxFloat32, -math.MaxFloat32, -math.MaxFloat32}

	var value float32
	for n := 0; n < len(points); n++ {
		// Axis 0 = ( 1,  1,  1)
		value = points[n].X + points[n].Y + points[n].Z
		if value < d.Min[0] {
			d.Min[0] = value
