...
func TestSphericalToCartesian(t *testing.T) {
	...
	for i, test := range tests {
		if out := SphericalToCartesian(test.r, test.theta, test.phi); (!FloatEqualThreshold(out.X, test.out.X, 1e-4) && !(math.IsNaN(test.out.X) && math.IsNaN(out.X))) ||
			(!FloatEqualThreshold(out.Y, test.out.Y, 1e-4) && !(math.IsNaN(test.out.Y) && math.IsNaN(out.Y))) ||
			(!FloatEqualThreshold(out.Z, test.out.Z, 1e-4) && !(math.IsNaN(test.out.Z) && math.IsNaN(out.Z))) {
...
func TestCylindricalToCartesian(t *testing.T) {
	...
	for i, test := range tests {
		if out := CylindricalToCartesian(test.rho, test.phi, test.z); (!FloatEqualThreshold(out.X, test.out.X, 1e-4) && !(math.IsNaN(test.out.X) && math.IsNaN(out.X))) ||
			(!FloatEqualThreshold(out.Y, test.out.Y, 1e-4) && !(math.IsNaN(test.out.Y) && math.IsNaN(out.Y))) ||
			(!FloatEqualThreshold(out.Z, test.out.Z, 1e-4) && !(math.IsNaN(test.out.Z) && math.IsNaN(out.Z))) {
...
func TestSphericalToCylindrical(t *testing.T) {
	...
	for i, test := range tests {
		if rho, phi2, z := SphericalToCylindrical(test.r, test.theta, test.phi); (!FloatEqualThreshold(rho, test.out.X, 1e-4) && !(math.IsNaN(test.out.X) && math.IsNaN(rho))) ||
			(!FloatEqualThreshold(phi2, test.out.Y, 1e-4) && !(math.IsNaN(test.out.Y) && math.IsNaN(phi2))) ||
			(!FloatEqualThreshold(z, test.out.Z, 1e-4) && !(math.IsNaN(test.out.Z) && math.IsNaN(z))) {
...
func TestCylindricalToSpherical(t *testing.T) {
	...
	for i, test := range tests {
		if r, theta, phi2 := CylindricalToSpherical(test.rho, test.phi, test.z); (!FloatEqualThreshold(r, test.out.X, 1e-4) && !(math.IsNaN(test.out.X) && math.IsNaN(r))) ||
			(!FloatEqualThreshold(theta, test.out.Y, 1e-4) && !(math.IsNaN(test.out.Y) && math.IsNaN(theta))) ||
			(!FloatEqualThreshold(phi2, test.out.Z, 1e-4) && !(math.IsNaN(test.out.Z) && math.IsNaN(phi2))) {
