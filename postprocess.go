...
// InitPostProcessSystem will allocate all the resources required to make the
// Image Post Processing system work.
func InitPostProcessSystem() {
	...
	vertpos.Bind(gl2.ARRAY_BUFFER)
	vertpos.Data(gl2.ARRAY_BUFFER, len(_quadvertpos)*2*4, unsafe.Pointer(&_quadvertpos[0]), gl2.STATIC_DRAW)
	//gl.BufferData(gl.ARRAY_BUFFER, len(_quadvertpos)*2*4, unsafe.Pointer(&_quadvertpos[0]), gl.STATIC_DRAW)

	vertuv := gl2.GenBuffer()
	vertuv.Bind(gl2.ARRAY_BUFFER)
	vertuv.Data(gl2.ARRAY_BUFFER, len(_quadvertuv)*2*4, unsafe.Pointer(&_quadvertuv[0]), gl2.STATIC_DRAW)
...
// PostProcessFramebufferer is an interface to represent a single PostProcess
// effect.
...
func NewPostProcessFramebuffer(width, height int32, fragmentSource string) (*PostProcessFramebuffer, error) {
	...
	if gl.CheckFramebufferStatus(gl2.FRAMEBUFFER) != gl2.FRAMEBUFFER_COMPLETE {
		return &ppf, errors.New("framebuffer incomplete")
	}
	return &ppf, nil
}

// PreRender binds either the next post process fbo if there is one or unbinds
// any fbo to render to screen. Also disable depth test.
func (ppfb *PostProcessFramebuffer) PreRender() {
	if ppfb.next != nil {
		ppfb.Fb.Bind(gl2.FRAMEBUFFER)
	} else {
		ppfb.Fb.Unbind(gl2.FRAMEBUFFER)
	}
...
// Render takes a texture and feed it to the fragment shader as a fullscreen
// texture. It will call the next post process pass if there is one.
...
// SetNext sets the post process effect to pass automatically after this post
// process.
...
// Fstri draws a fullscreen triangle such that it covers the entire screen with
// uv coordinates from [0,0]-[1,1]
func Fstri() {
	fstri.vao.Bind()
	defer fstri.vao.Unbind()
	gl.EnableVertexAttribArray(0)
	fstri.pos.Bind(gl2.ARRAY_BUFFER)
	fstri.vao.VertexAttribPointer(0, 2, gl2.FLOAT, false, 0, nil)
	//gl.VertexAttribPointer(0, 2, gl.FLOAT, false, 0, nil)

	gl.EnableVertexAttribArray(1)
	fstri.uv.Bind(gl2.ARRAY_BUFFER)
	fstri.vao.VertexAttribPointer(1, 2, gl2.FLOAT, false, 0, nil)
	//gl.VertexAttribPointer(1, 2, gl.FLOAT, false, 0, nil)

	gl.DrawArrays(gl2.TRIANGLES, 0, 3)
}
